...
bool elements::tab(const char* name, bool boolean)
{
    ...
    */
    ...
    if (hovered) {
        window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor(1.0f, 1.0f, 1.0f, 0.025f), 3.0f);
        window->DrawList->AddText(tab_icons, 15.0f, (rect.Min + rect.Max - label_size) / 2, ImColor(0.61f, 0.f, 0.81f), name);
    }
    else {
        window->DrawList->AddText(tab_icons, 15.0f, (rect.Min + rect.Max - label_size) / 2, ImColor(1.f, 1.f, 1.f), name);
    }
...
bool elements::subtab(const char* name, bool boolean)
{
    ...
    if (hovered) {
        window->DrawList->AddText(ImVec2(rect.Min.x + 15, (rect.Min.y + rect.Max.y) / 2 - label_size.y / 2), ImColor(0.61f, 0.f, 0.81f), name);
        window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor(1.0f, 1.0f, 1.0f, 0.025f), 3.0f);
    }
    else {
        window->DrawList->AddText(ImVec2(rect.Min.x + 15, (rect.Min.y + rect.Max.y) / 2 - label_size.y / 2), ImColor(1.f, 1.f, 1.f), name);
    }
